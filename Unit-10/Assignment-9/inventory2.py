...
def get_items():
    items = []
    with open("Unit-10/Assignment-9/all_items.txt", 'r') as file:
...
def save(inventory):
    with open("Unit-10/Assignment-9/inventory.txt", "w") as file:
        file.writelines(inventory)

    
def get_inventory():
    STARTING_INV = ["a wooden staff\n", "a wizard hat\n", "a cloak of invisibility\n"]
    with open("Unit-10/Assignment-9/inventory.txt", "w") as file:
        file.writelines(STARTING_INV)
        return STARTING_INV
...
def walk(inventory, items):
    ...
    if input("Do you want to grab it? (y/n): ") == "y":
        if len(inventory) < 4:
            ...
            print(f"You picked up a {items[num]}") 
            save(inventory) 
        else:
